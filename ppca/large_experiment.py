...
if __name__ == '__main__':
    ...
    for totalN in [1000,10000]:
        for D in [10,500]:#[20,30]: #
            trueK = 7#D//3
            K = trueK
            trueinit = get_h_and_v_params(K,D,experimental_condition = 0, prior_std = 1)
            ...
            for model_prior_std in [1.3,1.7]: #[1,3]
                ...
                for experimental_condition in [4]:#range(n_experimental_conditions):
                    for K in range(1,Kmax+1):
                        ...
                        if experimental_condition > 0 and experimental_condition < 4 and K == 1:
                            K1model = "{}_factors_{}_dataseed_{}_initseed_{}_N_{}_D_{}_priorstd_{}.p".format(1,0, args.dataseed, args.initseed,N,D,model_prior_std)
                            _,_,_,param_history,_,_ = pickle.load(open(K1model, 'rb'))
                            continue
                        elif experimental_condition == 0:
                            param_history = None
                        filename = "{}_factors_{}_dataseed_{}_initseed_{}_N_{}_D_{}_priorstd_{}.p".format(K,str(experimental_condition), args.dataseed, args.initseed,N,D,model_prior_std)
