...
def p_value_of_slope(loss, window):
    if len(loss) < window or any(map(torch.isinf, loss)):
        return 0
    else:
        recent = loss[-window:]
...
def inference(model, guide, training_data, test_data, init, n_iter = 10000, window = 500, batch_size = 10, n_mc_samples = 16, learning_rate = 0.1, learning_rate_decay = 0.9999, n_posterior_samples = 800, slope_significance = 0.5):
    ...
    while p_value_of_slope(lppds,10) < slope_significance and i < n_iter:
        try:
            loss = svi.step(training_data, batch_size, init)
